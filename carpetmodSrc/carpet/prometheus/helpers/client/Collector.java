...
public abstract class Collector {
    ...
    public static class MetricFamilySamples {
        ...
        public MetricFamilySamples(String name, String unit,Type type, String help, List<MetricFamilySamples.Sample> samples) {
            if (!unit.isEmpty() && !name.endsWith("_" + unit)) {
                ...
                List<MetricFamilySamples.Sample> mungedSamples = samples;
                if (type == Type.COUNTER) {
                    ...
                    MetricFamilySamples.Sample s;
                    String n;
                    for (Iterator var8 = samples.iterator(); var8.hasNext(); ((List) mungedSamples).add(new MetricFamilySamples.Sample(n, s.labelNames, s.labelValues, s.value, s.exemplar, s.timestampMs))) {
                        s = (MetricFamilySamples.Sample) var8.next();
                        n = s.name;
                        if (name.equals(n)) {
                            n = withTotal;
                        }
        ...
        public MetricFamilySamples filter(Predicate<String> sampleNameFilter) {
            if (sampleNameFilter == null) {
                ...
                List<MetricFamilySamples.Sample> remainingSamples = new ArrayList(this.samples.size());
                Iterator var3 = this.samples.iterator();

                while (var3.hasNext()) {
                    MetricFamilySamples.Sample sample = (MetricFamilySamples.Sample) var3.next();
