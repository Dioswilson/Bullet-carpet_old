...
public class Gauge extends SimpleCollector<Gauge.Child> implements Describable {
    Gauge(Gauge.Builder b) {
        super(b);
    }

    public static Gauge.Builder build(String name, String help) {
        return (Gauge.Builder)((Gauge.Builder)(new Gauge.Builder()).name(name)).help(help);
    }
    ...
    public void inc(double amt) {
        ((Gauge.Child)this.noLabelsChild).inc(amt);
    }
    ...
    public void dec(double amt) {
        ((Gauge.Child)this.noLabelsChild).dec(amt);
    }

    public void set(double val) {
        ((Gauge.Child)this.noLabelsChild).set(val);
    }

    public void setToCurrentTime() {
        ((Gauge.Child)this.noLabelsChild).setToCurrentTime();
    }

    public Gauge.Timer startTimer() {
        return ((Gauge.Child)this.noLabelsChild).startTimer();
    }

    public double setToTime(Runnable timeable) {
        return ((Gauge.Child)this.noLabelsChild).setToTime(timeable);
    }
    ...
    public double get() {
        return ((Gauge.Child)this.noLabelsChild).get();
    }
    ...
    public List<MetricFamilySamples> collect() {
        ...
        while(var2.hasNext()) {
            Map.Entry<List<String>, Gauge.Child> c = (Map.Entry)var2.next();
            samples.add(new MetricFamilySamples.Sample(this.fullname, this.labelNames, (List)c.getKey(), ((Gauge.Child)c.getValue()).get()));
        }
    ...
    public static class Child {
        ...
        static Gauge.TimeProvider timeProvider = new Gauge.TimeProvider();
        ...
        public double setToTime(Runnable timeable) {
            Gauge.Timer timer = this.startTimer();
    ...
    public static class Timer implements Closeable {
        ...
        private Timer(Gauge.Child child) {
            this.child = child;
            this.start = Gauge.Child.timeProvider.nanoTime();
        }

        public double setDuration() {
            double elapsed = (double)(Gauge.Child.timeProvider.nanoTime() - this.start) / 1.0E9D;
    ...
    public static class Builder extends SimpleCollector.Builder<Gauge.Builder,Gauge> {
        public Builder() {
        }
