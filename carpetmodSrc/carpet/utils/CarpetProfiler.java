...
public class CarpetProfiler {
    ...
    public static void prepare_tick_report(int ticks) {
        ...
        time_repo.put("overworld.village", 0L);
        ...
        time_repo.put("the_nether.village", 0L);
        ...
        time_repo.put("the_end.village", 0L);
    ...
    public static void finalize_tick_report_for_time(MinecraftServer server) {
        ...
        accumulated += time_repo.get("overworld.village");
        Messenger.print_server_message(server, String.format(" - Village: %.3fms", divider * time_repo.get("overworld.village")));
        ...
        accumulated += time_repo.get("the_nether.village");
        Messenger.print_server_message(server, String.format(" - Village: %.3fms", divider * time_repo.get("the_nether.village")));
        ...
        accumulated += time_repo.get("the_end.village");
        Messenger.print_server_message(server, String.format(" - Village: %.3fms", divider * time_repo.get("the_end.village")));
    ...
    public static void finalize_tick_report_for_entities(MinecraftServer server) {
        ...
        List<Map.Entry<String, Long>> counts = time_repo.entrySet().stream()
                .filter((e) -> e.getKey().startsWith("c."))
                .sorted(Map.Entry.comparingByValue(Comparator.reverseOrder()))
                .collect(Collectors.toList());
        Map<String, Long> otherC = new HashMap<>();
        int index = 0;
        for (Map.Entry<String, Long> entry : counts) {
            if (index++ < 12) {
                ...
                String[] parts = entry.getKey().split("\\.");
                String dim = parts[1];
                otherC.merge(dim, entry.getValue(), Long::sum);
            }
        }
        otherC.entrySet().stream()
                .sorted(Map.Entry.comparingByValue(Comparator.reverseOrder()))
                .forEach((entry) -> Messenger.print_server_message(server, String.format(" - Rest in %s: %.3f",
                        entry.getKey(), (double) entry.getValue() / tick_health_requested)));

        Messenger.print_server_message(server, "Top 10 grossing:");
        List<Map.Entry<String, Long>> ticks = time_repo.entrySet().stream()
                .filter((e) -> e.getKey().startsWith("t."))
                .sorted(Map.Entry.comparingByValue(Comparator.reverseOrder()))
                .collect(Collectors.toList());
        Map<String, Long> otherT = new HashMap<>();
        index = 0;
        for (Map.Entry<String, Long> entry : ticks) {
            if (index++ < 12) {
                String[] parts = entry.getKey().split("\\.");
                String dim = parts[1];
                String name = parts[2];
                ...
                String[] parts = entry.getKey().split("\\.");
                String dim = parts[1];
                otherT.merge(dim, entry.getValue(), Long::sum);
            }
        }
        otherT.entrySet().stream()
                .sorted(Map.Entry.comparingByValue(Comparator.reverseOrder()))
                .forEach((entry) -> Messenger.print_server_message(server, String.format(" - Rest in %s: %.3fms",
